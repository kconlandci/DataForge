...
const UID_STORAGE_KEY = "DataForge_firebase_uid";
      ...
      if (user) {
        try {
          ...
          if (storedUid && storedUid !== currentUid) {
            console.warn(
              `[DataForge] UID changed: stored=${storedUid}, current=${currentUid}. ` +
          ...
          console.error("[DataForge] UID persistence failed:", error);
        } finally {
          setIsAuthReady(true);
        }
      } else {
        ...
        try {
          ...
          console.error("[DataForge] Anonymous sign-in failed:", error);
          setUid(null);
          setIsAuthReady(true);
        }
      }
